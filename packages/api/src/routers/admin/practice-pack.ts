...
const updatePack = admin
	...
	.handler(async ({ input }) => {
		const packId = typeof input.id === "string" ? Number.parseInt(input.id) : input.id;
...
const updateQuestion = admin
	...
	.handler(async ({ input }) => {
		const questionId = typeof input.id === "string" ? Number.parseInt(input.id) : input.id;
...
const addQuestionToPack = admin
	...
	.handler(async ({ input }) => {
		const packId = typeof input.practicePackId === "string" 
...
const removeQuestionFromPack = admin
	...
	.handler(async ({ input }) => {
		const packId = typeof input.practicePackId === "string" 
			? Number.parseInt(input.practicePackId) 
			: input.practicePackId;
		const questionId = typeof input.questionId === "string" 
...
const updateAnswerOption = admin
	...
	.handler(async ({ input }) => {
		const answerId = typeof input.id === "string" ? Number.parseInt(input.id) : input.id;
