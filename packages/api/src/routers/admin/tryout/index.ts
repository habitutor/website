  ...
  .handler(async ({ input }) => {
    try {
      ...
      import("fs").then((fs) =>
        fs.appendFileSync(
          "error.log",
          "CREATE SOAL ERROR: " + (error instanceof Error ? error.stack : String(error)) + "\n",
        ),
      );
  ...
  .handler(async ({ input }) => {
    try {
      ...
      import("fs").then((fs) =>
        fs.appendFileSync(
          "error.log",
          "CREATE PILIHAN ERROR: " + (error instanceof Error ? error.stack : String(error)) + "\n",
        ),
      );
      console.error("CREATE PILIHAN ERROR:", error);
