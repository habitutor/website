...
export const tryoutRepo = {
  ...
  getTryoutDetail: async ({ db = defaultDb, tryoutId }: { db?: DrizzleDatabase; tryoutId: string }) => {
  ...
  listSubtesByTryout: async ({ db = defaultDb, tryoutId }: { db?: DrizzleDatabase; tryoutId: string }) => {
  ...
  getSubtesDetail: async ({ db = defaultDb, subtesId }: { db?: DrizzleDatabase; subtesId: string }) => {
  ...
  listSoalBySubtes: async ({ db = defaultDb, subtesId }: { db?: DrizzleDatabase; subtesId: string }) => {
  ...
  getSoalDetail: async ({ db = defaultDb, soalId }: { db?: DrizzleDatabase; soalId: string }) => {
  ...
  listPilihanBySoal: async ({ db = defaultDb, soalId }: { db?: DrizzleDatabase; soalId: string }) => {
  ...
  deleteTryout: async ({ db = defaultDb, tryoutId }: { db?: DrizzleDatabase; tryoutId: string }) => {
    ...
    if (existing.status === "published") {
      throw new TryoutError(
        "CONFLICT",
        "Tryout yang sudah dipublish tidak dapat dihapus. Batalkan publish terlebih dahulu.",
      );
    }
    ...
    const [deleted] = await db.delete(tryout).where(eq(tryout.id, tryoutId)).returning();
  ...
  deleteSubtes: async ({ db = defaultDb, subtesId }: { db?: DrizzleDatabase; subtesId: string }) => {
    const soalList = await db.query.tryoutSoal.findMany({
      where: eq(tryoutSoal.subtesId, subtesId),
      with: {
        pilihanJawaban: {
    ...
    const [deleted] = await db.delete(tryoutSubtes).where(eq(tryoutSubtes.id, subtesId)).returning();
  ...
  createSoal: async ({
    ...
    const uploadedPembahasanGambar = pembahasanGambar
      ? await uploadTryoutImage(pembahasanGambar, "habitutor/tryout/Soal")
      : null;
  ...
  deleteSoal: async ({ db = defaultDb, soalId }: { db?: DrizzleDatabase; soalId: string }) => {
    const existing = await db.query.tryoutSoal.findFirst({
      where: eq(tryoutSoal.id, soalId),
      with: {
        pilihanJawaban: {
    ...
    const [deleted] = await db.delete(tryoutSoal).where(eq(tryoutSoal.id, soalId)).returning();
  ...
  deletePilihanJawaban: async ({ db = defaultDb, pilihanId }: { db?: DrizzleDatabase; pilihanId: string }) => {
    const existing = await db.query.tryoutPilihanJawaban.findFirst({
      where: eq(tryoutPilihanJawaban.id, pilihanId),
      columns: { gambarUrl: true },
    });
    ...
    const [deleted] = await db.delete(tryoutPilihanJawaban).where(eq(tryoutPilihanJawaban.id, pilihanId)).returning();

    if (!deleted) throw new TryoutError("NOT_FOUND", "Pilihan jawaban tidak ditemukan");
    return deleted;
  },

  publishTryout: async ({ db = defaultDb, tryoutId }: { db?: DrizzleDatabase; tryoutId: string }) => {
    const [updated] = await db
      .update(tryout)
      .set({
        status: "published",
  ...
  unpublishTryout: async ({ db = defaultDb, tryoutId }: { db?: DrizzleDatabase; tryoutId: string }) => {
    const [updated] = await db
      .update(tryout)
      .set({
        status: "draft",
   ...
   */
  getQuestions: async ({ db = defaultDb, sesiSubtesId }: { db?: DrizzleDatabase; sesiSubtesId: string }) => {
  ...
  submitSubtest: async ({
    ...
    const jawabanList = await db
      .select()
      .from(tryoutJawaban)
      .innerJoin(tryoutSesiSoal, eq(tryoutJawaban.sesiSoalId, tryoutSesiSoal.id))
      .where(eq(tryoutSesiSoal.sesiSubtesId, sesiSubtesId));
    ...
    if (nextSubtes) {
      ...
      const existingNextSesiSubtes = await db.query.tryoutSesiSubtes.findFirst({
        where: and(eq(tryoutSesiSubtes.sesiId, sesiSubtes.sesi.id), eq(tryoutSesiSubtes.subtesId, nextSubtes.id)),
      });
   ...
   */
  getResult: async ({ db = defaultDb, sesiId }: { db?: DrizzleDatabase; sesiId: string }) => {
    ...
    const ranking = allSesi.findIndex((s) => s.id === sesiId) + 1;
   ...
   */
  autoSubmitSubtest: async ({ db = defaultDb, sesiSubtesId }: { db?: DrizzleDatabase; sesiSubtesId: string }) => {
