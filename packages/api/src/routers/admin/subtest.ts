...
const upsertVideo = authed
	...
	.handler(async ({ input }) => {
		...
		// Validate video type
		if (input.videoType === "upload" && !input.videoFilePath) {
			throw new ORPCError("BAD_REQUEST", {
				message: "Video file path wajib diisi untuk tipe upload",
			});
		}

		if (input.videoType === "link" && !input.videoUrl) {
			throw new ORPCError("BAD_REQUEST", {
				message: "Video URL wajib diisi untuk tipe link",
			});
		}
		...
		const [video] = await db
			...
			.values({
				...
				videoFilePath: input.videoFilePath || null,
...
const deleteVideo = authed
	...
	.handler(async ({ input }) => {
		...
		// TODO: Delete video file from storage if videoType is "upload"
