...
export const tryoutRepo = {
   ...
   */
  listPublishedTryouts: async ({ db = defaultDb }: { db?: DrizzleDatabase }) => {
   ...
   */
  getSesiSubtesInfo: async ({ db = defaultDb, sesiSubtesId }: { db?: DrizzleDatabase; sesiSubtesId: string }) => {
    const sesiSubtes = await db.query.tryoutSesiSubtes.findFirst({
      where: eq(tryoutSesiSubtes.id, sesiSubtesId),
      with: {
        subtes: {
   ...
   */
  getHistory: async ({ db = defaultDb, userId }: { db?: DrizzleDatabase; userId: string }) => {
  ...
  /**
   * Ambil daftar passing grade universitas dan program studi
   */
  getPassingGrade: async ({ db = defaultDb }: { db?: DrizzleDatabase }) => {
    return await db.query.universitas.findMany({
      orderBy: (univ) => [desc(univ.rankUniv)],
      with: {
        programStudi: true,
      },
    });
