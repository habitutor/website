...
import {
  ...
  dashboardLiveClass,
} from "@habitutor/db/schema/dashboard";
import { and, asc, desc, eq, sql } from "drizzle-orm";

export const dashboardRepo = {
  cleanupExpiredLiveClasses: async ({
    db = defaultDb,
  }: {
    db?: DrizzleDatabase;
  }) => {
    await db
      .delete(dashboardLiveClass)
      .where(
        sql`(${dashboardLiveClass.date} + ${dashboardLiveClass.time}) < now()`,
      );
  },
  ...
  listPublishedAnnouncements: async ({
    ...
    return db
      ...
      .where(
        and(
          eq(dashboardAnnouncement.isPublished, true),
          eq(dashboardAnnouncement.variant, "primary"),
        ),
      )
  ...
  listLiveClasses: async ({
    ...
    return db
      ...
      .where(
        and(
          eq(dashboardLiveClass.isPublished, true),
          onlyThreeX ? eq(dashboardLiveClass.access, "3x") : undefined,
        ),
      )
