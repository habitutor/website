  ...
  const defaultRenderCard = (item: CarouselItem) => {
    ...
    const itemId =
      typeof item.id === "number"
        ? item.id
        : parseInt(item.id as string, 10) || 0;
    const bgColor =
      itemId % 2 === 0
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between flex-1 flex-col p-4">
        ...
        {showNavigation && (
          <>
            ...
            <button
              ...
              className="bg-primary-200 absolute top-1/2 z-20 hidden -translate-x-49.5 -translate-y-1/2 rounded-[10px] p-2.5 text-neutral-100 shadow transition-all duration-300 hover:scale-110 hover:bg-neutral-800 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 lg:flex xl:-translate-x-53.75"
        ...
        <Container className="pt-5 pb-0">
          <div className="mb-6 flex w-full items-center justify-between gap-8 lg:hidden">
            <button
              onClick={prevSlide}
              disabled={isTransitioning}
              className="bg-primary-200 rounded-[10px] p-2.5 text-neutral-100 transition-all duration-300 hover:scale-110 hover:bg-neutral-800 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
            >
              <ArrowLeftIcon size={24} />
            </button>
            ...
            return (
              <button
                ...
                className={`size-2 rounded-full transition-all duration-200 disabled:cursor-not-allowed ${
                  index === activeIndex
                    ? "bg-dot-active scale-125"
                    : "bg-dot-inactive scale-125"
                }`}
              />
            );
          })}
