	...
	const defaultRenderCard = (item: CarouselItem) => {
		...
		return (
			<div className="mx-auto flex h-full w-full max-w-100 flex-col overflow-hidden rounded-xl border border-primary-100 shadow-sm transition">
				<div className="relative flex flex-1 flex-col justify-between overflow-hidden text-pretty p-4 text-left">
					<p className="max-h-full overflow-y-auto font-medium text-sm">{item.desc}</p>
				...
				<div className="mt-4 flex gap-2">
					{Array.from({
						...
						<button
							...
							className={`h-2 w-2 rounded-full ${i === current ? "scale-125 bg-dot-active" : "bg-dot-inactive"}`}
						/>
					))}
				</div>
			)}
