...
function RouteComponent() {
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			...
			const contentValue = value.content && typeof value.content === "object" ? value.content : {};
		...
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				...
				<div className="flex gap-4">
					<form.Subscribe>
						{(state) => (
							<Button type="submit" size="sm" disabled={!state.canSubmit || saveMutation.isPending}>
					...
					{(field) => (
						<div className="space-y-2">
							...
							{field.state.meta.errors.map((error, index) => {
								...
								return (
									<p key={`${field.name}-error-${index}`} className="text-red-500 text-sm">
										{errorMessage}
									</p>
								);
							})}
              ...
              /> */}
							...
							<TiptapSimpleEditor
								...
								onChange={(content) => {
									...
									field.handleChange(content && typeof content === "object" ? content : {});
								}}
							/>
