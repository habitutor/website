...
export const FlashcardCard = () => {
  ...
  function handleAnswerSelect(answerId: number) {
    saveAnswer({
      questionId: data!.assignedQuestions[currentPage - 1].question.id,
      answerId,
    });
    ...
    if (currentPage === data?.assignedQuestions.length) {
      ...
      queryClient.invalidateQueries({
        queryKey: orpc.flashcard.streak.queryKey(),
      });
        ...
        {data?.assignedQuestions[currentPage - 1].question.answerOptions.map(
          (option, i) => (
            <button
              ...
              className="inline-flex items-center gap-3 rounded-md border border-secondary bg-white p-4 text-start text-foreground"
            >
