...
function TryoutPage() {
	const { data: tryouts = [], isPending } = useQuery(
		orpc.tryout.list.queryOptions({ input: {} }),
	);
					...
					{isPending ? (
						...
						<div className="flex flex-col items-center justify-center rounded-xl border bg-white py-16 shadow-sm">
							...
							<p className="text-sm text-muted-foreground">
								Tryout akan segera hadir. Nantikan ya!
							</p>
						</div>
					) : (
						<div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
				...
				<TabsContent value="passing_grade">
					<div className="flex flex-col gap-4">
						...
						<div className="overflow-hidden rounded-md border bg-white shadow-sm">
							<Table>
								<TableHeader className="bg-blue-50/50">
									<TableRow>
										<TableHead className="font-semibold text-primary-300">
											Universitas
										</TableHead>
										<TableHead className="hidden font-semibold text-primary-300 sm:table-cell">
											Rank ↑
										</TableHead>
										<TableHead className="font-semibold text-primary-300">
											Jurusan
						...
						<div className="text-xs text-muted-foreground">
							0 of {PASSING_GRADE_DATA.length} row(s) selected.
						</div>
					</div>
				</TabsContent>
	...
	return (
		<div className="group flex flex-col justify-between gap-4 rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:border-primary-300/50 hover:shadow-md">
			<div className="flex flex-col gap-2">
				<h3 className="text-lg font-bold text-gray-900 group-hover:text-primary-300">
					{tryout.judul}
				</h3>
				{tryout.deskripsi && (
...
function HistoryTabContent() {
	const { data: history = [], isPending } = useQuery(
		orpc.tryout.history.queryOptions({ input: {} }),
	);
	...
	return (
		<div className="group flex flex-col justify-between gap-4 rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:border-primary-300/50 hover:shadow-md">
			<div className="flex items-center justify-between">
				<h3 className="text-lg font-bold text-gray-900 truncate pr-2" title={history.judul}>
					{history.judul}
				</h3>
				<span
