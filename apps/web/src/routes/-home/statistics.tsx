...
import { ArrowUpLeftIcon, BookOpenIcon, CloudLightningIcon, FileIcon, Frown, Pocket } from "lucide-react";
import { motion } from "motion/react";

export function Statistics() {
	return (
		<section className="relative flex flex-col items-center justify-center overflow-visible bg-neutral-100">
			<div className="container relative flex w-full flex-col gap-40 items-centerlg:gap-20">
				<motion.div
				...
				<div className="container relative flex w-full flex-col items-center gap-40 px-4 lg:gap-20">
					<motion.div
						className="pointer-events-none absolute -bottom-10 left-40 hidden size-80 rounded-full border-tertiary-200 border-t-2 bg-linear-to-b from-background to-transparent lg:block"
						initial={{ opacity: 0 }}
						whileInView={{ opacity: 1 }}
						viewport={{ once: true }}
						transition={{ delay: 0.9, duration: 0.3 }}
					/>
					<motion.div
						className="pointer-events-none absolute right-35 -bottom-10 hidden size-80 rounded-full border-tertiary-200 border-t-2 bg-linear-to-b from-background to-transparent lg:block"
						initial={{ opacity: 0 }}
						whileInView={{ opacity: 1 }}
						viewport={{ once: true }}
						transition={{ delay: 0.9, duration: 0.3 }}
					/>
					<motion.div
						className="pointer-events-none absolute bottom-0 size-73 rounded-full bg-linear-to-b from-50% from-secondary-200 to-50% to-transparent md:-bottom-20 lg:top-60 lg:size-156"
						initial={{ opacity: 0 }}
						whileInView={{ opacity: 1 }}
						viewport={{ once: true }}
						transition={{ delay: 0.9, duration: 0.3 }}
					...
					>
						<h2 className="text-center font-bold text-2xl *:text-pretty lg:text-4xl">
					...
					<div className="flex w-full flex-col items-center justify-center gap-2 lg:gap-10">
						<div className="relative z-2 flex w-full flex-col gap-4 overflow-hidden rounded-2xl border-2 border-primary-400 bg-primary-300 px-1 py-8 text-neutral-100 lg:w-7/10 lg:px-10">
							<p className="z-2 text-center font-bold text-xl lg:text-3xl">
