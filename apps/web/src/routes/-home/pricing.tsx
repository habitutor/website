...
function StarterCard({ data }: { data: typeof DATA.pricing.starter }) {
	return (
		<div className="rounded-2xl bg-white p-6 shadow-sm">
			...
			<Link to="/" className="mt-6 inline-flex items-center gap-2 rounded-lg border px-4 py-2 text-sm">
				{data.cta}
...
function PremiumCard({ data }: { data: typeof DATA.pricing.premium }) {
	return (
		<div className="relative rounded-2xl bg-blue-700 p-6 text-white shadow-sm">
			...
			<Link to="/" className="mt-6 inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-blue-700 text-sm">
				{data.cta}
				<ArrowRightIcon size={16} />
			</Link>
		</div>
