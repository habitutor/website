      ...
      {filteredPacks && filteredPacks.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed p-12 text-center">
          ...
          <h3 className="mb-2 font-semibold text-lg">
            No practice packs found
          </h3>
          <p className="mb-4 text-muted-foreground text-sm">
            {searchQuery
              ? "Try adjusting your search query"
              : "Get started by creating a new practice pack"}
          </p>
          {!searchQuery && (
            <Button asChild>
              <Link to="/admin/practice-packs/create">
          ...
          <div className="flex flex-wrap items-center justify-center gap-1 sm:gap-1">
            {page > 3 && (
              <>
                ...
                {page > 4 && (
                  <span className="hidden px-1 text-xs sm:inline sm:px-2">
                    ...
                  </span>
                )}
              </>
...
function PracticePackCard({
  pack,
}: {
  pack: { id: number; title: string; description: string | null };
}) {
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete "{pack.title}"? This action cannot
              be undone and will remove all associated questions.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
