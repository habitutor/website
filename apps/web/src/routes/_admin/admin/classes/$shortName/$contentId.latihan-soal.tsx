            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  {existingQuestions.length} soal sudah terhubung dengan konten
                  ini
                </p>
              </div>
            </div>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <Checkbox
                        checked={isSelected}
                        onCheckedChange={() =>
                          handleQuestionToggle(question.questionId)
                        }
                      />
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                {practicePacks.data?.data.map(
                  (pack: { id: number; title: string }) => (
          ...
          {packQuestions.data?.questions &&
            packQuestions.data.questions.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <Label>
                    Pilih Soal Baru (dipilih: {selectedQuestionIds.length})
                  </Label>
                  {packQuestions.data.questions.length > 0 && (
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          setSelectedQuestionIds((prev) =>
                            prev.filter((id) => !packQuestionIds.includes(id))
                          );
                        }}
                      >
                ...
                <div className="max-h-125 space-y-2 overflow-y-auto">
                  {packQuestions.data.questions.map(
                    (question: {
                      ...
                      const isLinked = existingQuestionIds.includes(
                        question.id
                      );
                      const isSelected = selectedQuestionIds.includes(
                        question.id
