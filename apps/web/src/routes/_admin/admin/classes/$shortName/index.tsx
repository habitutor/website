    ...
    onSubmit: async ({ value }) => {
      ...
      const items =
        createDialogType === "material"
          ? materialContents.data
          : tipsContents.data;
      const maxOrder =
        items && items.length > 0
          ? Math.max(...items.map((i) => i.order ?? 0))
          : 0;

      // For now we ensure at least one component exists by always requiring a note on creation
      if (!value.withNote) {
        toast.error(
          "Konten baru harus memiliki minimal satu komponen. Aktifkan catatan materi.",
        );
        return;
      }
      ...
      <div className="space-y-4">
        <Tabs defaultValue="material">
          <TabsList>
            ...
            <TabsTrigger value="tips_and_trick">Tips & Trick</TabsTrigger>
          </TabsList>
        ...
        <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Buat konten baru untuk{" "}
                {createDialogType === "material" ? "Materi" : "Tips & Trick"}
              </DialogDescription>
            </DialogHeader>
            <form
              onSubmit={(e) => {
                ...
                {(field) => (
                  <div className="flex items-center gap-2">
                    ...
                    <Label htmlFor={field.name} className="font-normal text-sm">
                      Buat catatan materi awal (minimal satu komponen per
                      konten)
                    </Label>
                  </div>
                )}
