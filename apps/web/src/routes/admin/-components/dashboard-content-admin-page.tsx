    ...
    <AdminContainer>
      ...
      {activeTab === "live-class" ? (
        <Card className="space-y-4 p-6">
          <h3 className="font-semibold text-lg">Tambah Live Class</h3>
          ...
          <div className="space-y-3 pt-4">
            {contentQuery.data?.liveClasses.map((item) => (
              <LiveClassRow
                key={item.id}
                item={item}
                onDelete={(id) => deleteLiveClassMutation.mutate({ id })}
              />
            ))}
          </div>
        </Card>
          ...
          {primaryAnnouncement ? (
            <div className="space-y-3">
              ...
              <Textarea
                ...
                onChange={(event) =>
                  setAnnouncementForm((prev) => ({ ...prev, description: event.target.value }))
                }
              />
              <Button
                onClick={() =>
